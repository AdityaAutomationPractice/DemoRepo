...
Feature: Negitive scenarios for postman workspace
  ...
  Scenario Outline: Get all the workspaces in my account without providing the authentication
  When  send the Workspace end point without authentication
  Then Validate the status code is 401
  And Validate the response for the request errors messages "<errorname>" "<message>"
  Examples:
    |errorname|message|
    |AuthenticationError|Invalid API Key. Every request requires a valid API Key to be sent.|
